...
export class BM25Engine {
  ...
  private docs = new Map<string, { tokens: string[]; memory: Memory }>();
  private df = new Map<string, number>(); // term → doc frequency

  add(memory: Memory): void {
    if (this.docs.has(memory.id)) this.remove(memory.id);
  ...
  search(query: string, options: SearchOptions = {}): Array<{ id: string; score: number }> {
...
export function extractSnippet(content: string, query: string, maxLen = 220): string {
