 ...
 * over stdio transport (default) — compatible with Cursor, Claude Code, Windsurf.
 */
...
const vaultPath = process.env['OPENVAULT_PATH'] ?? DEFAULT_VAULT_PATH;
...
await vault.load().catch(err => {
    process.stderr.write(`[openvault] load warning: ${err.message}\n`);
});
...
// ─── vault_search ─────────────────────────────────────────────────────────────
server.tool('vault_search', 'Search memories using hybrid BM25 + semantic search (RRF fusion). Returns ranked results with snippets.', {
    ...
    const text = results
        .map((r, i) => `[${i + 1}] [${r.memory.category}] ${r.memory.title}\n${r.snippet}\n` +
        `id: ${r.memory.id} | score: ${r.score.toFixed(4)}`)
        .join('\n\n');
    return { content: [{ type: 'text', text }] };
});
// ─── vault_write ──────────────────────────────────────────────────────────────
server.tool('vault_write', 'Store a new memory. Category is auto-detected from content if not specified.', {
...
// ─── vault_forget ─────────────────────────────────────────────────────────────
server.tool('vault_forget', 'Delete a memory by finding it via search. First call with confirm=false to preview, then confirm=true to delete.', {
...
// ─── vault_status ─────────────────────────────────────────────────────────────
server.tool('vault_status', 'Return vault statistics: total memory count, per-category breakdown, last write time.', {}, async () => {
    const stats = vault.stats();
    const breakdown = Object.keys(stats.categories).length > 0
...
// ─── vault_context ────────────────────────────────────────────────────────────
server.tool('vault_context', 'Return relevant memories for the current project or working directory. Call this at the start of a session.', {
